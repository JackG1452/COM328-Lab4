...
template <class T>
Sorter<T>::Sorter(int newSize = DEF_SIZE) : elements(newSize) {}
	...
	if (!in_stream.fail()) {
		...
		while (in_stream >> x) {
			if (i == elements.length())
				elements.changeSize(elements.length()*2); // double size of array
			elements[i++] = x;
		}
		// resize array to i, to remove unused blanks
		if (i>0) elements.changeSize(i);
...
void Sorter<T>::writeFile(const std::string & fname) {
	...
	if (!out_stream.fail()) {
		for(int i=0; i<elements.length(); i++)
			out_stream << elements[i] << std::endl;
	}
	out_stream.close();
}
