...
private:
	...
	void merge(Array<T> & work, Array<T> & data, int leftS, int rightS, int rightE);
...
template <class T>
Sorter<T>::Sorter(int size) : elements(size) {}
...
int Sorter<T>::binarySearch(const T & element)
{
	...
	while (left <= right) {
		...
		else if (elements[pivot] < element) {
...
void Sorter<T>::selectionSort() {
	...
	for (int marker = 0; marker < elements.length() - 1; marker++) {
		...
		moves += 3; // increment moves (3 in a swap)
...
template <class T>
void Sorter<T>::merge(Array<T> & work, Array<T> & data, int leftS, int rightS, int rightE) {
	int leftE = rightS - 1;
	int tmpPos = leftS;
